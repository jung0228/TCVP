...
사용법:
1. Letsur Gateway API 키 설정: export LETSUR_API_KEY='your-key-here'
   (선택) 기본 모델 변경: export LETSUR_MODEL='원하는_모델_ID'
...
class SegmentIntegrator:
    def __init__(self, api_key: str = None, model: str = None):
        if api_key:
            ...
            raise ValueError("Letsur AI Gateway API 키가 필요합니다. 환경변수 LETSUR_API_KEY를 설정하거나 직접 전달하세요.")
...
def main():
    ...
    parser.add_argument('-k', '--api-key', help='Letseng AI Gateway API 키')
    parser.add_argument('-m', '--model', help='사용할 게이트웨이 모델 ID (기본: gpt-4.1)')
    ...
    if not args.input_path:
        ...
        print("옵션:")
        print("  -m/--model MODEL_ID   사용할 모델 지정 (기본: gpt-4.1 또는 LETSUR_MODEL)")
        print("\n환경변수 설정:")
        print("  export LETSUR_API_KEY='your-key-here'")
        print("  export LETSUR_MODEL='gpt-4.1'  # 선택 사항")
        return
    
    try:
        integrator = SegmentIntegrator(api_key=args.api_key, model=args.model)
